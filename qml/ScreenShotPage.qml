...
Item {
    ...
    function copyToClipboard() {
        view.copyToClipboard(Qt.rect(
                                 selectLayer.x * Screen.devicePixelRatio,
                                 selectLayer.y * Screen.devicePixelRatio,
                                 selectLayer.width * Screen.devicePixelRatio,
                                 selectLayer.height * Screen.devicePixelRatio))
    }
    ...
    Rectangle {
        id: sizeToolTip
        visible: selectLayer.visible && selectLayer.width > 1
                 && selectLayer.height > 1
        ...
        Label {
            ...
            text: "%1 * %2".arg(parseInt(selectLayer.width)).arg(
                      parseInt(selectLayer.height))
        }
    }
    ...
    Rectangle {
        ...
        y: {
            ...
            // 选中区域与工具栏高度大于总高度
            if (selectLayer.y + selectLayer.height + tools.height
                    + FishUI.Units.smallSpacing >= control.height) {
    ...
    MouseArea {
        ...
        onPositionChanged: {
            ...
            if (mouseX >= selectLayer.newX) {
                selectLayer.width = mouseX
                        < (control.x + control.width) ? (mouseX - selectLayer.x) : selectLayer.width
            } else {
